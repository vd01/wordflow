...
App({
  ...
  onLaunch(options) {
    ...
    // When user scans a wxacode.getUnlimited QR code:
    //   options.scene = numeric entry type (e.g. 1047 = scanned mini program code)
    //   options.query.scene = the custom scene string we embedded in the QR code
    let scene = null
    if (options && options.query && options.query.scene) {
      // QR code with custom scene parameter (from wxacode.getUnlimited)
      ...
      console.log('App: received scene from QR code query:', scene)
    }
    if (scene) {
  ...
  onShow(options) {
    // Handle QR code scan when mini program is already running
    if (options && options.query && options.query.scene) {
      let scene = options.query.scene
      try {
        scene = decodeURIComponent(scene)
      } catch (e) {}
      console.log('App: received scene from QR code on show:', scene)
      if (scene && !this.globalData.pendingScene) {
        this.globalData.pendingScene = scene
        // Notify current page if it's the index page
        const pages = getCurrentPages()
        if (pages.length > 0 && pages[0].handleSceneLogin) {
          pages[0].handleSceneLogin(scene)
        }
      }
    }
  },
